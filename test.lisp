...
(defun auto-clear-cache ()
  (let ((manager (create-cache-manager :max-size 100))
        (removed-list))
    (add-cache manager (list "a1" 10) "x1" 20 (lambda () (push "x1" removed-list)))
    (sleep 0.01)
    (add-cache manager (list "a2" 10) "x2" 20 (lambda () (push "x2" removed-list)))
    (sleep 0.01)
    (add-cache manager (list "a3" 10) "x3" 20 (lambda () (push "x3" removed-list)))
    (sleep 0.01)
    (add-cache manager (list "a4" 10) "x4" 20 (lambda () (push "x4" removed-list)))
    (sleep 0.01)
    (add-cache manager (list "a5" 10) "x5" 21 (lambda () (push "x5" removed-list)))
    (sleep 0.01)
    ...
    (assert (equal '("x4" "x3" "x2") removed-list))))
